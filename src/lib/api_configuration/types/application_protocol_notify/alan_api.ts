...
export abstract class AlanNode extends AlanStruct {
	protected _root:Capplication_protocol_notify|undefined;
...
export class Capplication_protocol_notify extends AlanNode {
	...
	constructor(init:Tapplication_protocol_notify) {
		super();
		const $this = this;
		this.properties = {
...
/* de(resolution) */
function auto_defer_validator<T extends (...args:any) => void>(root:Capplication_protocol_notify, callback:T):T {
	return callback;
}
...
export namespace Capplication_protocol_notify {
	export function create(init:Tapplication_protocol_notify):Capplication_protocol_notify {
		const instance = new Capplication_protocol_notify(init);
		finalize_application_protocol_notify(instance);
		;
		return instance;
	};
}
