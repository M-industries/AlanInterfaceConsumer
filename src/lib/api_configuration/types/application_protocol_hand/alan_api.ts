...
export namespace Capplication_protocol_hand {
	export function create(init:Tapplication_protocol_hand, lazy_eval:boolean = false):Capplication_protocol_hand {
		const instance = new Capplication_protocol_hand(init, lazy_eval);
		if (!lazy_eval) resolve_application_protocol_hand(instance);
		return instance;
	};
}
